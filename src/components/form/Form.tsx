...
export const FormComponent = ({ typeForm }: { typeForm: string }) => {
  const { registerSubmit, loading, loginSubmit } = useContext(ClientContext);
  ...
  const {
    ...
    resolver: zodResolver(
      typeForm === "register" ? clientSchema : clientLoginSchema
    ),
  });
    ...
    <>
      {typeForm === "register" ? (
        ...
        >
          ...
          <div className="flex flex-row gap-2">
            <span className="text-sm text-black">Não possui uma conta?</span>
            <Link href="/register" className="text-sm text-[#1A4CC7]">
              Registre-se
            </Link>
          </div>
          <div className="flex w-full justify-center">
